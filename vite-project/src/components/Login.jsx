...
export default function Login() {
    ...
    const handleSignupDB = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:6900/HCM', data);
            console.log("Server Response:", response.data);
            alert("Signup successful!");

            navigate('/otp1' , {state : {email : data.email}})
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center" style={{
            ...
            <div className="w-11/12 max-w-md md:max-w-2xl h-full bg-[#0a0a0a] text-white rounded-3xl shadow-[0_0_40px_rgba(255,0,0,0.7)] p-6 md:p-10 transition-all duration-300 ease-in-out font-['Creepster'] relative overflow-hidden">
             ...
             <Link to = "/otp1">  <h1 className="text-3xl md:text-4xl text-center text-red-600 py-5 leading-snug drop-shadow-[2px_2px_8px_rgba(255,0,0,0.8)] animate-pulse">
                ...
                <form className="space-y-7" onSubmit={handleSignupDB}>
