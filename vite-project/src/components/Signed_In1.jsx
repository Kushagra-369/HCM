...
export default function Signed_In1() {
  ...
  return (
    <div className={`min-h-screen items-center font-mono transition-colors duration-500 ${isDark ? 'bg-gradient-to-br from-black via-blue-900 to-black text-red-100' : 'bg-gradient-to-br from-gray-100 via-white to-gray-200 text-gray-900'}`}>
      <div className="mb-6 flex justify-end p-4 w-full">
        <button onClick={toggleTheme} className={`px-4 py-2 rounded-lg font-semibold shadow-md ${isDark ? 'bg-white text-black hover:bg-gray-300' : 'bg-black text-white hover:bg-gray-800'}`}>
          {isDark ? 'Light Mode' : 'Dark Mode'}
        </button>
      </div>

      <div className="flex justify-center items-center min-h-[80vh]">
        <form onSubmit={formik.handleSubmit} className={`border-2 shadow-2xl px-8 py-10 w-[400px] rounded-xl transition-colors duration-500 ${isDark ? 'border-red-700 shadow-black bg-gradient-to-b from-gray-900 via-black to-red-900' : 'border-gray-400 shadow-gray-400 bg-white'}`}>
          <h1 className={`text-center text-5xl font-bold drop-shadow-lg tracking-widest ${isDark ? 'text-red-600' : 'text-blue-600'}`}>LOG-IN</h1>

          {SignData.map((item, key) => (
            <div key={key} className="py-4">
              <label htmlFor={item.name} className={`block mb-2 text-lg uppercase tracking-wider ${isDark ? 'text-red-300' : 'text-gray-700'}`}>{item.name}</label>
              <input
                id={item.name}
                name={item.name}
                type={item.type}
          ...
          <div className="flex justify-between px-1 text-sm mt-2">
            <span></span>
            <Link to="/forgot-password" className={`hover:underline ${isDark ? 'hover:text-blue-400' : 'hover:text-blue-600'}`}>Forgot password?</Link>
          </div>

          <div className="pt-6">
            <button type="submit" disabled={isButtonDisabled} className={`w-full h-12 rounded-lg font-bold tracking-wide transition shadow-md ${isButtonDisabled ? 'bg-gray-400 cursor-not-allowed' : isDark ? 'bg-red-800 hover:bg-red-600 shadow-red-800 hover:shadow-red-500 text-white' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-400 text-white'}`}>
