...
export default function ProfileD() {
    ...
    const user = {
        name: userData.name || 'Kushagra',
        email: userData.email || 'kushagra@gmail.com',
    ...
    const LogOutUser = () => {
        ...
        // Step 1: Redirect first
        navigate('/');

        // Step 2: After short delay, clear login state
        setTimeout(() => {
            setIsLog(false);
            setUserDashboard(false);
            localStorage.removeItem('isLog');
            localStorage.removeItem('UserDashboard');
        }, 100); // 100ms delay to allow route to change first
    };
                            ...
                            {({ active }) =>
                                item.link ? (
                                    <Link
                                        to={item.link}
                                        onClick={item.click}
                                        className={`flex items-center gap-3 px-4 py-2 text-sm ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'
                                            }`}
                                    >
                                    ...
                                    <button
                                        onClick={item.click}
                                        className={`flex w-full items-center gap-3 px-4 py-2 text-sm text-left ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'
                                            }`}
                                    >
                                        <Icon className="h-5 w-5" />
                                        {item.name}
